...
export const BlogPostList = () => {
  const { data } = useSWR<{ blogPosts: BlogPost[] }>('/api/blog-posts', fetcher, {
          ...
          <Card key={blogPost.id}>
            <CardHeader className="flex flex-row">
              ...
              {blogPost.status === 'needs approval' && (
                <div className="flex">
                  <Badge variant="secondary">An AI revision need approval</Badge>
                </div>
              )}
              {blogPost.ai_publishing_recommendations && (
                <div className="flex">
                  <Badge variant="secondary">Publishing recommendations are available</Badge>
                </div>
              )}
            </CardHeader>
            <CardContent>
