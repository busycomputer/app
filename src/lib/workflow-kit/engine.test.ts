...
describe('Engine.graph', () => {
  ...
  it('validates unknown action kinds', () => {
    const wf: Workflow = {
      actions: [{ id: 'a', kind: 'not-found-in-engine' }],
      edges: [{ from: '$source', to: 'a' }],
    }

    expect(() => engine.graph(wf)).toThrow('Workflow instance references unknown action kind')
  })
  ...
  it('validates that there are source edges', () => {
    ...
    expect(() => engine.graph(wf)).toThrow('Workflow has no starting actions')
  })
  ...
  it('validates that there are no disconnected actions', () => {
    ...
    expect(() => engine.graph(wf)).toThrow('An action is disconnected and will never run: b')
  })
...
describe('ExecutionState.interpolate', () => {
  it('correctly references current state and events', () => {
    ...
    expect(state.interpolate(`{"==": [{"var": "!ref($.state.action_a)"}, "a"}`)).toEqual(
      `{"==": [{"var": "test"}, "a"}`
    )
    ...
    // Object walking.
    expect(state.interpolate({ '==': ['!ref($.state.action_b)', 'a'] })).toEqual({
      '==': [{ ok: true }, 'a'],
    })
    expect(
      state.interpolate({
