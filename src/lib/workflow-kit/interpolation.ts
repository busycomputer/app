...
export function interpolate(value: any, vars: Record<string, any>) {
  ...
  while (foundRefs.length > 0) {
    ...
    const { path: jsonPath, ref: substr } = foundRefs.shift() ?? { path: '', ref: '' }
...
function interpolateObject(value: object, vars: Record<string, any>) {
  ...
  const stack: Array<{ obj: Record<string, any>; key: string | null }> = [
    { obj: result, key: null },
  ]
  ...
  while (stack.length > 0) {
    ...
    if (isRef(value)) {
      ...
      const foundRefs = refs(value)
      while (foundRefs.length > 0) {
        const { path, ref } = foundRefs.shift()!
