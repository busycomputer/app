  ...
  const workflow: Workflow = {
    ...
    edges: [
      ...
      // if-a should evaluate to true so this never runs.
      {
        from: 'if-a',
        to: 'stepB-false',
        conditional: { type: 'else', ref: '!ref($.output.result)' },
      },
      ...
      // Check that "match" works with non-string values.
      {
        from: 'stepB-true',
        to: 'stepC-true',
        conditional: { type: 'match', ref: '!ref($.output)', value: 42 * 99 },
      },
      // This should never run, as the value is not equal (type equality).
      {
        from: 'stepB-true',
        to: 'stepC-false',
        conditional: { type: 'match', ref: '!ref($.output)', value: (42 * 99).toString() },
      },
    ],
  }
